...
/**
 * @brief Nitrokey-specific OATH device implementation
 *
 * Single Responsibility: Handles communication with ONE Nitrokey OATH application.
 * Each instance manages connection, authentication, and operations for a specific device.
 *
 * Nitrokey-specific behavior:
 * - Creates NitrokeyOathSession internally (uses LIST+CALCULATE, CALCULATE_ALL optional)
 * - Serial number in SELECT response (TAG_SERIAL_NUMBER 0x8F)
 * - Touch required status word: 0x6982 (SecurityStatusNotSatisfied)
 *
 * This class encapsulates all state and operations for a single Nitrokey device,
 ...
 */
class NitrokeyOathDevice : public OathDevice
{
    Q_OBJECT

public:
    /**
     * @brief Constructs Nitrokey OATH device instance
     *
     ...
     * @param challenge Challenge from Nitrokey SELECT
     ...
     */
    explicit NitrokeyOathDevice(const QString &deviceId,
    ...
    // - Derived class shadow signal (NitrokeyOathDevice::credentialCacheFetched) is emitted
...
protected:
    /**
     * @brief Factory method for creating temporary Nitrokey session
     *
     * Creates a NitrokeyOathSession instance for temporary use during reconnect.
     * Implementation of pure virtual from OathDevice base class.
     *
     * @param handle PC/SC card handle
     * @param protocol PC/SC protocol
     * @return Nitrokey-specific session instance (cast to base type)
     */
